...
| Problem rediscovery | Report that the assumed pain was not observed, then supply a newly observed Who and Pain | Keep the current Story authoritative until discovery input exists, reject solution language, preserve the disproven account as a losing record, and show all downstream impact before Layer 2 approval | `test_unobserved_problem_opens_discovery_then_governs_a_layer_two_revision`, `test_rediscovery_candidate_preserves_the_disproven_problem_as_a_loss`, `test_rediscovery_rejects_solution_language` |
